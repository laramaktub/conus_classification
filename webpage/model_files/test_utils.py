...
def data_augmentation(im_list, mode='minimal', tags=None, params=None, im_size=224,
...
def single_prediction(test_func, im_list, aug_params=None, crop_mode='random'):
    ...
    for i, im in enumerate(im_list):
        ...
        try:
            if crop_mode == 'random':
                batch = data_augmentation([im]*10, mode='minimal', **aug_params)
    ...
    label_list=synsets[args].tolist()
    pred_list=pred_prob[args].tolist()
    if (label_list[0]=="The image is probably not a conus" and pred_list[0] > 0.70):
        args=args[:1] 
    elif ("The image is probably not a conus" in label_list):
        index = label_list.index("The image is probably not a conus")
        args=np.delete(args, index)
    pred_lab = args
